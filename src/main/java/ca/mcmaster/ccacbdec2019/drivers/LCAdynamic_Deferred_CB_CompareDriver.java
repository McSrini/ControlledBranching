...
import static ca.mcmaster.ccacbdec2019.Parameters.SOLUTION_RUN_TIME_IN_HOURS;
 ...
 * @author tamvadss
 */
public class LCAdynamic_Deferred_CB_CompareDriver {
       
    private static Logger logger = Logger.getLogger(LCAdynamic_Deferred_CB_CompareDriver.class); 
     
    static {
        logger.setLevel( LOGGING_LEVEL);
        PatternLayout layout = new PatternLayout("%5p  %d  %F  %L  %m%n");     
        try {
            RollingFileAppender rfa =new  RollingFileAppender(layout,LOG_FOLDER+LCAdynamic_Deferred_CB_CompareDriver.class.getSimpleName()+ LOG_FILE_EXTENSION);
    ...
    public static void main(String[] args) throws IloException {
        ...
        logger.info ("number of nonleaf nodes in the CB instruction tree is " + iTree.getNumberOfNonLeafNodes() );
         ...
         //TEST 1 : solve the merged tree - note that we are using deferred execution
        ...
        logger.info (" ramp up solution used as cutoff "+ bestSolutionFoundByRampup) ;
        reincarnator.reincarnate(bestSolutionFoundByRampup,  SOLUTION_RUN_TIME_IN_HOURS );                
        logger.info ("Reincarnator end   "  ) ;
                
        
        //TEST 2: round robin thru the collection of leafs
        logger.info ("round robin start") ;
        Worker worker = new Worker (individualSubProblems_for_RoundRobin ) ;
