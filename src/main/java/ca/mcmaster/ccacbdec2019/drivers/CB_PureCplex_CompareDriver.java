 ...
 * @author tamvadss
 */
public class CB_PureCplex_CompareDriver {
       
    private static Logger logger = Logger.getLogger(CB_PureCplex_CompareDriver.class); 
     
    static {
        logger.setLevel( LOGGING_LEVEL);
        PatternLayout layout = new PatternLayout("%5p  %d  %F  %L  %m%n");     
        try {
            RollingFileAppender rfa =new  RollingFileAppender(layout,LOG_FOLDER+CB_PureCplex_CompareDriver.class.getSimpleName()+ LOG_FILE_EXTENSION);
    ...
    public static void main(String[] args) throws IloException {
        ...
        if (rampup.isFeasible()) {
            logger.info (" ramp up solution used as cutoff "+ rampup.getBestKnownSolution()) ;
            reincarnator.reincarnate(rampup.getBestKnownSolution());
        }else {
            logger.info (" ramp up did not find any feasible solution to use as cutoff ");
            reincarnator.reincarnate();
        }        
        logger.info ("Reincarnator end , number of node redirects " + reincarnator.getNumberofNodeRedirects()) ;
        
        //we can discard the ramped up tree
        rampup.end();
        
               
        logger.info("Starting merged problem solve with worker");
        IloCplex reincarnatedTree = reincarnator.getMergedTree();
        logger.info ("reincarnated tree has leafcount "+reincarnatedTree.getNnodesLeft64());
        Worker worker = new Worker (reincarnator.getMergedTree()) ;
        worker.solveMergedProblem();
        logger.info("Completed merged problem solve with worker");
